...
export default function PlayerPage() {
  ...
  const shareTrack = async () => {
    ...
    if (navigator.share) {
      try { await navigator.share({ title: `${currentMedia.title} en Resona`, url }); } catch {}
    } else {
      try {
  ...
  const reportTrack = () => {
    ...
    setReportReason('');
    setReportDetails('');
    toast('Denuncia enviada', 'success');
  };
  ...
  if (!currentMedia) {
    return (
      <div className="flex h-full flex-col items-center justify-center p-6 text-center">
        ...
        <p className="mt-2 text-sm text-muted">Empieza a reproducir una canción.</p>
        <button onClick={() => navigate(-1)} className="mt-6 inline-flex items-center gap-2 rounded-full border border-line bg-surface/60 px-5 py-2.5 text-sm font-semibold text-muted transition hover:border-fuchsia-400/40 hover:text-fuchsia-300">
  ...
  const art = currentMedia.kind === 'music' ? imageUrl(currentMedia.track.artwork, '1000x1000') : currentMedia.poster;
  const subtitle = currentMedia.kind === 'music' ? currentMedia.subtitle : mediaSubtitle(currentMedia);
  const isFav = currentMedia.kind === 'music' ? favorites.some((f) => f.id === currentMedia.track.id) : isVodFav;
    ...
    <div className="flex h-full animate-fade-in">
      ...
      <div className="relative flex h-full w-1/2 flex-col items-center justify-between p-6" style={{ background: art && !isVideo ? 'linear-gradient(180deg, rgba(0,0,0,0.7) 0%, rgba(30,10,40,0.9) 40%, rgba(15,5,20,1) 100%)' : 'linear-gradient(180deg, rgba(20,10,30,0.95) 0%, rgba(10,5,15,1) 100%)' }}>
        ...
        <div className="relative z-10 flex w-full max-w-lg items-center justify-between">
          ...
          <span className="text-xs font-semibold uppercase tracking-wider text-white/50">{isVideo ? (currentMedia.kind === 'movie' ? 'Película' : 'Serie') : 'Reproduciendo'}</span>
        ...
        <div className="relative z-10 w-full max-w-lg space-y-5">
          ...
          <div className="flex items-center justify-center gap-5">
            <button onClick={handleFavorite} className="rounded-full p-2.5 text-white/60 transition hover:text-fuchsia-400">
              <Heart className={cn('h-5 w-5', isFav && 'fill-fuchsia-400 text-fuchsia-400')} />
            </button>
            <button onClick={prev} className="rounded-full p-2.5 text-white/80 transition hover:text-white">
              <SkipBack className="h-7 w-7 fill-current" />
            </button>
            <button onClick={togglePlay} className="flex h-16 w-16 items-center justify-center rounded-full bg-white text-surface shadow-xl shadow-white/20 transition-all hover:scale-105 active:scale-95">
              {isLoading ? <div className="h-7 w-7 animate-spin rounded-full border-3 border-surface border-t-transparent" /> : isPlaying ? <Pause className="h-7 w-7 fill-current" /> : <Play className="ml-1 h-7 w-7 fill-current" />}
            </button>
            <button onClick={next} className="rounded-full p-2.5 text-white/80 transition hover:text-white">
        ...
        {showQueue ? (
          ...
          <div className="flex flex-1 flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-2">
              {loadingSimilar ? (
                <div className="space-y-2 p-4">{Array.from({ length: 6 }).map((_, i) => <div key={i} className="flex items-center gap-3 rounded-xl p-2"><div className="h-12 w-12 animate-pulse rounded-lg bg-surface-2" /><div className="flex-1 space-y-2"><div className="h-4 w-3/4 animate-pulse rounded bg-surface-2" /><div className="h-3 w-1/2 animate-pulse rounded bg-surface-2" /></div></div>)}</div>
                  ...
                  return (
                    <div key={track.id} onClick={() => playTrack(track)} className={cn('group flex items-center gap-3 rounded-xl p-2 transition cursor-pointer', isCurrentTrack ? 'bg-fuchsia-500/15 border border-fuchsia-400/30' : 'hover:bg-surface-2 border border-transparent')}>
                      <div className="relative h-12 w-12 shrink-0 overflow-hidden rounded-lg">
                        {trackArt ? <img src={trackArt} alt="" className="h-full w-full object-cover" /> : <div className="flex h-full w-full items-center justify-center bg-surface-3"><Music2 className="h-5 w-5 text-fuchsia-300/50" /></div>}
                        <div className="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 transition group-hover:opacity-100"><Play className="h-5 w-5 fill-white text-white" /></div>
                        {isCurrentTrack && isPlaying && <div className="absolute bottom-1 right-1 flex gap-0.5"><div className="h-2 w-0.5 animate-pulse bg-fuchsia-400" /><div className="h-3 w-0.5 animate-pulse bg-fuchsia-400" style={{ animationDelay: '0.15s' }} /><div className="h-2 w-0.5 animate-pulse bg-fuchsia-400" style={{ animationDelay: '0.3s' }} /></div>}
                      </div>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-3xl border border-line bg-surface p-6 shadow-2xl shadow-black/40">
            <div className="mb-5 flex items-center justify-between">
              ...
              <button onClick={() => { setShowReportModal(false); setReportReason(''); setReportDetails(''); }} className="text-faint transition hover:text-text"><X className="h-5 w-5" /></button>
            </div>
            <div className="mb-4 flex items-center gap-3 rounded-2xl bg-surface-2 p-3">
              {art && <img src={art} alt="" className="h-12 w-12 rounded-lg object-cover" />}
              <div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-text">{currentMedia.title}</p><p className="truncate text-xs text-muted">{subtitle}</p></div>
            </div>
            <div className="space-y-2">
              {['Contenido inapropiado', 'Violación de derechos de autor', 'Spam o fraude', 'Otro'].map((reason) => (
                <button key={reason} onClick={() => setReportReason(reason)} className={cn('w-full rounded-xl border px-4 py-2.5 text-left text-sm transition', reportReason === reason ? 'border-red-400/50 bg-red-500/15 text-red-300' : 'border-line text-muted hover:border-red-400/30 hover:text-text')}>{reason}</button>
              ))}
            </div>
            {reportReason && <div className="mt-4"><label className="mb-1 block text-xs font-semibold text-muted">Detalles (opcional)</label><textarea value={reportDetails} onChange={(e) => setReportDetails(e.target.value)} placeholder="Describe el problema..." rows={3} className="w-full resize-none rounded-xl border border-line bg-surface-2 px-4 py-2.5 text-sm text-text placeholder-faint outline-none transition focus:border-red-400/40" /></div>}
            <div className="mt-5 flex gap-3">
              <button onClick={() => { setShowReportModal(false); setReportReason(''); setReportDetails(''); }} className="flex-1 rounded-full border border-line py-2.5 text-sm font-semibold text-muted transition hover:text-text">Cancelar</button>
              <button onClick={reportTrack} disabled={!reportReason.trim()} className="flex-1 rounded-full bg-red-600 py-2.5 text-sm font-bold text-white transition hover:bg-red-700 disabled:opacity-30">Denunciar</button>
            </div>
          </div>
        </div>
      )}
